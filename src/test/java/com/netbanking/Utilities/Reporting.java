...
public class Reporting extends TestListenerAdapter
{
	...
	public void onStart(ITestContext testContext)
	{
		...
		extent.setSystemInfo("user","VIGNESH RAJ");
	...
	public void onTestFailure(ITestResult tr)
	{
		...
		Rectangle rectangle = new Rectangle(scDimension);
		BufferedImage srcfile=robot.createScreenCapture(rectangle);
		Date currentdate = new Date();

		String scrfdatefile = currentdate.toString().replace(" ", "-").replace(":", "-");

		File desfile = new File(System.getProperty("user.dir")+"\\Screenshot\\"+scrfdatefile+".png");
		try {
		...
		{
		try {
			logger.fail("Screenshot is below:" + logger.addScreenCaptureFromPath(System.getProperty("user.dir")+"\\Screenshot\\"+scrfdatefile+".png"));
			} 
		catch (IOException e) 
				{
				e.printStackTrace();
